...
class NucleotideTransformerDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if file==1 :
            ...
            # tfsite_base_path = Path("/public/ojsys/eye/wuwencan/lihaokai/dataset/hyena/tfbs_individual/back_165_copy/")
            # 训练和测试文件路径
        ...
        #  elif file == "test1":
        #     tfsite_base_path = Path("/public/ojsys/eye/wuwencan/lihaokai/dataset/gwas/tfbs_gla")
        #     bed_file = tfsite_base_path / f"{dataset_name}.bed"

        #     print(f"[INFO] 数据集BED文件路径: {bed_file}")

        #     if not bed_file.exists():
        #         raise FileNotFoundError(f"BED文件不存在: {bed_file}")

        #     # 读取BED文件：第4列为序列，第5列为索引（假设无表头）
        #     try:
        #         df = pd.read_csv(bed_file, sep='\t', header=None, usecols=[3, 4], names=['sequence', 'index'])
        #         sequences = df['sequence'].astype(str).str.strip().tolist()
        #         indices = df['index'].tolist()  # 索引列（可以是字符串或整数）
        #     except Exception as e:
        #         raise RuntimeError(f"读取BED文件失败: {e}")

        #     # 标签可根据需要设置：这里保留之前的标签递增逻辑（每个样本标签=索引值）或固定为1
        #     # 例如，若标签也为索引值（从1开始递增）：
        #     # all_data = [(seq, idx) for idx, seq in zip(indices, sequences)]  # 标签=索引
        #     # 或者标签固定为1：
        #     all_data = [(seq, 0) for seq in sequences]

        #     print(f"[INFO] 总共读取 {len(all_data)} 个样本")

        #     # 存储序列和标签，同时保留索引列表
        #     self.seqs = all_data
        #     self.seq_indices = indices  # 新增属性，存储每个样本的标识符
        ...
        #         pass
        #     else:
        #         print(f"[WARNING] split='{split}' 要求的数据集不存在，所有数据仅用作测试集。")
        #         self.seqs = []
        #         self.seq_indices = []

        #     self.label_mapper = {
        #         i: (indices[i], label)  # 使用索引列作为标识符
        #         for i, (sequence, label) in enumerate(self.seqs)
        #     }
