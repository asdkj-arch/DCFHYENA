...
from src.dataloaders.datasets.hg38_kmer_tokenizer import KmerTokenizer
...
class HG38(SequenceDataset):
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            ...
            print("**Using custom KmerTokenizer**")


            k = 3  # 你可以从 config 中读取这个参数，也可以设为默认
            from itertools import product
            kmers = [''.join(p) for p in product('ACGT', repeat=k)]

            self.tokenizer = KmerTokenizer(kmers=kmers, k=k, model_max_length=self.max_length)
            # print("KmerTokenizer vocab size:", tokenizer.vocab_size)
            
            self.vocab_size = len(self.tokenizer)
        elif self.tokenizer_name == 'dual':
            print("**using dual channel **")
            ...
            self.kmer_tokenizer = KmerTokenizer(kmers=kmers, k=k, model_max_length=self.max_length)

            # 封装为字典
            self.tokenizer = ( self.char_tokenizer, self.kmer_tokenizer )

            self.vocab_size = (len(self.char_tokenizer), len(self.kmer_tokenizer))  # optional
            
            
        elif self.tokenizer_name == 'dual1':
            print("**using dual1 channel **")
            self.char_tokenizer= CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
            ...
            self.bpe_tokenizer = AutoTokenizer.from_pretrained('AIRI-Institute/gena-lm-bert-base')

            # 封装为字典
            self.tokenizer = ( self.char_tokenizer, self.bpe_tokenizer )

            self.vocab_size = (len(self.char_tokenizer), len(self.bpe_tokenizer))  # optional
            
        elif self.tokenizer_name == 'dual2':
            print("**using dual2 channel **")
            ...
            # 封装为字典
            self.tokenizer = ( self.kmer_tokenizer, self.bpe_tokenizer )

            self.vocab_size = (len(self.kmer_tokenizer), len(self.bpe_tokenizer))  # optional
    ...
    def init_datasets(self):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            HG38Dataset(split=split,
                        ...
                        kmer_tokenizer=self.tokenizer[0] if self.tokenizer_name == "dual2" else self.tokenizer[1],
                        bpe_tokenizer=self.tokenizer[1] if self.tokenizer_name == "dual2" else self.tokenizer[1],
...
class GenomicBenchmark(HG38):
    ...
    def __init__(self, dataset_name, dest_path=None, tokenizer_name=None, d_output=None, rc_aug=False,
...
#         if getattr(self, "val_only", False):  # 你也可以换成 config.train.test
#             # 只加载 test
#             self.dataset_test = NucleotideTransformerDataset(
#                 split="test",
...
#         if getattr(self, "dataset_val", None) is None:
#             return []
...
#             return self._data_loader(self.dataset_test, batch_size=self.batch_size_eval, shuffle=False)
#         return self._data_loader(self.dataset_val, batch_size=self.batch_size_eval, shuffle=self.shuffle_eval)
