...
class SequenceLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        #     in_dim1=128, in_dim2=128, k_dim=128, v_dim=128, num_heads=4
    ...
    def setup(self, stage=None):
        ...
        if 'mxdna' in str(self.hparams.model).lower():
            assert self.hparams.model.d_model == self.hparams.model.config.hidden_size, "d_model必须与hidden_size一致"
            assert all(ks % 2 == 1 for ks in self.hparams.model.config.expert_kernel_sizes), "卷积核尺寸需为奇数"
        ...
        if "model_aux" in self.hparams:
            print("构建 model_aux...,,model_aux不为空")
            self.model_aux = utils.instantiate(registry.model, self.hparams.model_aux)
        ...
        self.final_val_torchmetrics = self.task.test_torchmetrics
    ...
    def forward(self, batch):
        ...
        bidfusion=self.bidfusion,
        gatefusion=self.gatefusion,

                    
    )
        # return self.task.forward(batch, self.encoder, self.model, self.model1 ,self.decoder, self._state)
    ...
    def _shared_step(self, batch, batch_idx, prefix="train"):
        ...
        if countt == 1:
            ...
            if x.ndim == 3:  # (batch, seq_len, num_classes)
                probs = F.softmax(x, dim=-1)              # (batch, seq_len, num_classes)
                preds = probs.argmax(dim=-1)              # (batch, seq_len)
                pos_probs = probs[..., 1]                 # 取正类概率 (batch, seq_len)
            else:  # (batch, num_classes)
                probs = F.softmax(x, dim=-1)              # (batch, num_classes)
                preds = probs.argmax(dim=-1)              # (batch,)
                pos_probs = probs[..., 1]                 # 取正类概率 (batch,)


            # print("Probs:", probs.detach().cpu().numpy())
            # print("Preds:", preds.detach().cpu().numpy())
            # print("Labels:", y.detach().cpu().numpy())
            save_path = "score.csv"


            if batch_idx == 0 and not os.path.exists(save_path):
                with open(save_path, "w") as f:
                    f.write("pred,label,prob\n")


            with open(save_path, "a") as f:
                for p, l, pr in zip(preds.detach().cpu().numpy().flatten(),
                                    y.detach().cpu().numpy().flatten(),
                                    pos_probs.detach().cpu().numpy().flatten()):
                    f.write(f"{p},{l},{pr:.6f}\n")
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        # prefix = self.test_loader_names[dataloader_idx].replace("/", "_")
    ...
    def configure_optimizers(self):
        ...
        all_params = list(self.parameters())
             ...
             # 如果有 model_aux，也加入参数列表
        if hasattr(self, "model_aux") and self.model_aux is not None:
            # 只加入 requires_grad=True 的参数
            aux_params = [p for p in self.model_aux.parameters() if p.requires_grad]
            all_params += aux_params

        # 过滤掉标记了 _optim 的参数（保持你原来的逻辑）
        params = [p for p in all_params if not hasattr(p, "_optim")]

        # 构建 optimizer
        optimizer = utils.instantiate(registry.optimizer, self.hparams.optimizer, params)

        # # 打印检查 model_aux 参数是否加入 optimizer
        # opt_params = set(p for g in optimizer.param_groups for p in g['params'])
        # if hasattr(self, "model_aux") and self.model_aux is not None:
        #     print("=== model_aux 参数检查 ===")
        #     for name, p in self.model_aux.named_parameters():
        #         if p.requires_grad:
        #             if p in opt_params:
        #                 print(f"[OK] {name} 已在 optimizer 中")
        #             else:
        #                 print(f"[WARN] {name} 没有加入 optimizer！")
        #         else:
        #             print(f"[SKIP] {name} 不可训练")
...
#         optimizer = utils.instantiate(registry.optimizer, self.hparams.optimizer, params)
    ...
    def test_dataloader(self):
        ...
        # self.test_loader_names = ["final_" + name for name in test_loader_names]
...
def train(config):
    ...
    print("model_aux embedding mean:", model.model_aux.backbone.embeddings.word_embeddings.weight.mean().item())
    print("model_aux embedding std:", model.model_aux.backbone.embeddings.word_embeddings.weight.std().item())

    # 同时读取主模型的 embedding
    print("main model embedding mean:", model.model.backbone.embeddings.word_embeddings.weight.mean().item())
    print("main model embedding std:", model.model.backbone.embeddings.word_embeddings.weight.std().item())
    # model.model_aux = SequenceLightningModule(config)
    # model1 = SequenceLightningModule(config)
    # print(model)
    ...
    if config.train.get("pretrained_model_path", None) is not None:
        ...
        # print("这里是model.model_aux")
        
        # print(model.model_aux)
        ...
        if config.train.test:
            ...
            config_aux = deepcopy(config)
            config_aux.model_aux.vocab_size = 72  # 根据你的实际 model_aux vocab_size 修改

            # 3️⃣ 从 checkpoint 提取 model_aux 权重
            print("Loading model_aux weights...")
            ckpt_aux = torch.load(config.train.pretrained_model_path, map_location='cpu')
            state_dict_aux = ckpt_aux['state_dict']

            model_aux_state = {
                k.replace("model_aux.", ""): v
                for k, v in state_dict_aux.items()
                if k.startswith("model_aux.")
            }

            # 4️⃣ 根据 config_aux 创建空的 model_aux
            model_aux = DNAEmbeddingModel(
                d_model=config_aux.model_aux.d_model,
                n_layer=config_aux.model_aux.n_layer,
                d_inner=config_aux.model_aux.d_inner,
                vocab_size=config_aux.model_aux.vocab_size,
                resid_dropout=config_aux.model_aux.resid_dropout,
                embed_dropout=config_aux.model_aux.embed_dropout,
                fused_mlp=config_aux.model_aux.fused_mlp,
                fused_dropout_add_ln=config_aux.model_aux.fused_dropout_add_ln,
                residual_in_fp32=config_aux.model_aux.residual_in_fp32,
                pad_vocab_size_multiple=config_aux.model_aux.pad_vocab_size_multiple,
                layer=config_aux.model_aux.layer
            )

            # 5️⃣ 加载权重
            missing_keys, unexpected_keys = model_aux.load_state_dict(model_aux_state, strict=False)
            print(f"model_aux missing keys: {missing_keys}")
            print(f"model_aux unexpected keys: {unexpected_keys}")

            # 6️⃣ 挂到主模型
            model.model_aux = model_aux
            
            
            config1 = deepcopy(config)
            # config1.model.vocab_size = 12

            # 3️⃣ 从 checkpoint 提取 model_aux 权重
            print("Loading model weights...")
            ckpt = torch.load(config.train.pretrained_model_path, map_location='cpu')
            ...
            # 4️⃣ 根据 config_aux 创建空的 model_aux
            model1 = DNAEmbeddingModel(
                d_model=config1.model.d_model,
                n_layer=config1.model.n_layer,
                d_inner=config1.model.d_inner,
                vocab_size=config1.model.vocab_size,
                resid_dropout=config1.model.resid_dropout,
                embed_dropout=config1.model.embed_dropout,
                fused_mlp=config1.model.fused_mlp,
                fused_dropout_add_ln=config1.model.fused_dropout_add_ln,
                residual_in_fp32=config1.model.residual_in_fp32,
                pad_vocab_size_multiple=config1.model.pad_vocab_size_multiple,
                layer=config1.model.layer
            )

            # 5️⃣ 加载权重
            missing_keys, unexpected_keys = model1.load_state_dict(model_state, strict=False)
            print(f"model1 missing keys: {missing_keys}")
            print(f"model1 unexpected keys: {unexpected_keys}")
            ...
            # --- 加载 gatefusion ---
            gatefusion_state = {
                k.replace("gatefusion.", ""): v
                for k, v in state_dict.items()
                if k.startswith("gatefusion.")
            }
            model.gatefusion.load_state_dict(gatefusion_state, strict=False)

            # --- 加载 bidfusion ---
            bidfusion_state = {
                k.replace("bidfusion.", ""): v
                for k, v in state_dict.items()
                if k.startswith("bidfusion.")
            }
            model.bidfusion.load_state_dict(bidfusion_state, strict=False)


            
        else:
            config_aux = deepcopy(config)
            config_aux.model.vocab_size = 72

            print("Loading model_aux...")
            model_aux = SequenceLightningModule.load_from_checkpoint(
                config.train.pretrained_model_path_aux,
                config=config_aux,
                strict=False
            )
            model.encoder_aux = model_aux.encoder
            model.model_aux = model_aux.model
